...
7. **Empty results**: if a query returns no rows for the requested period,
   ...
   different period than what the user asked — do not offer a substitute.
...
10. **Pipeline & freshness**: Use Fivetran MCP to check connector state.
    Then ALWAYS also run `SELECT MAX(date) FROM agg_mall_daily` to report
...
11. **Statistical accuracy — CRITICAL**:
    ...
    - Average basket: ALWAYS `SUM(total_amount)/COUNT(invoice_no)` or
      `SUM(revenue)/SUM(transactions)`. NEVER `AVG(avg_basket)` — that is
      an average-of-averages and is statistically wrong.
...
7. **No-volunteer rule**: If a query returns no rows for the requested period,
   say "No data for [period]" and STOP. Never offer substitute figures from a
   different time period. Never state a number you did not retrieve from the DB.
8. **Basket size**: use `SUM(revenue)/SUM(transactions)` — never `AVG(avg_basket)`.
   **Unique customers**: use `COUNT(DISTINCT customer_id)` from fact_transactions.
...
5. **No-volunteer rule**: if a queried period has no data, say so and stop.
   NEVER substitute figures from a different period than requested.
